...
<template>
	<div class="app-container course-detail">
		<van-tabs color="#19B2FF">
			<van-tab title="视频">
				<div class="video-block" v-if="videoInfo.coverImg" @click="getVideoPlayAuth">
					<div class="content">
			...
			<van-tab title="课件">
				<div class="ebook-block" v-if="ebooksList.length > 0">
					<div class="ebook-item van-hairline--bottom" v-for="(item, index) in ebooksList" :key="index"
...
<script>
	...
	export default {
		...
		methods: {
			...
			queryDownloadUrl(objectKey) {
				this.$http({
					url: `/corp/course/pdfImg/${ objectKey}`,
					method: "get",
					params: {
						key: objectKey
					}
					...
					let imgArr = [];
					data.forEach(item => imgArr.push(item.imgUrl))
					ImagePreview(imgArr)
				})
			}
		},
