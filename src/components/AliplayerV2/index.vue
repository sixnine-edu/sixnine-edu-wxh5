<template>
    <div :id="playerId" v-loading="loading"></div>
</template>
<script>
export default {
    name: "VueAliplayerV2",
    ...
    methods: {
        ...
        init() {
            ...
            const scriptCommterID = "aliplayercomponents-min-js"
            const head = document.getElementsByTagName("head")
            const html = document.getElementsByTagName("html")
            let scriptTag = document.getElementById(scriptID)
            let linkIDTag = document.getElementById(linkID)
            let scriptCommterTag = document.getElementById(scriptCommterID)
            ...
            if (!scriptCommterTag) {
                scriptCommterTag = document.createElement("script")
                scriptCommterTag.type = "text/javascript"
                scriptCommterTag.id = scriptCommterID
                scriptCommterTag.src = this.scriptCommter
                html[0].appendChild(scriptCommterTag)
                //兼容单页加载和硬加载
                scriptCommterTag.addEventListener("load", () => {
                    setTimeout(() => {
                        this.initPlayer()
                    }, 1000)
                })
        ...
        initPlayer() {
            if (typeof window.Aliplayer != "undefined") {
                        ...
                        components: [
                            {
                                ...
                                args: this.config["bulletScreenComponentArgs"]
                            }
                        ]
                    },
                    player => {
                ...
                this.player.on("play", function(e) {
                    _video.removeEventListener("click", _play)
                    _video.addEventListener("click", _pause)
                })
                this.player.on("pause", function(e) {
                    _video.removeEventListener("click", _pause)
                    _video.addEventListener("click", _play)
                })
            }
        },
